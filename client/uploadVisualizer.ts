...
function handleInputChange(evt: Event){
    ...
    if(!uploadBox || !input.files) return;
    // Displaying files to be uploaded in list
    let list: Element | null = uploadBox.nextElementSibling;
...
function removeUpload(evt: MouseEvent){
    ...
    if(!deleteBtn) return;
    if(!deleteBtn.classList.contains("upload-remover") || !(input.files)) return;
    // Getting file name or id
    const row = deleteBtn.closest(".file-upload-row") as HTMLElement;
...
function removeFile(input: HTMLInputElement, fileIndex: string){
...
function createFileView(file: File, index: number) : HTMLDivElement {
